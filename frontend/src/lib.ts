...
export const statusLabel = (status: string) => ({ ready: "已就绪", pending: "等待中", processing: "正在评分", completed: "已完成", failed: "失败", unreadable: "无法读取", expired: "已过期" }[status] ?? status);
...
export const diffMarkerForStatus = (status: string) => ({ qualified: "+", unqualified: "-", selected: ">" }[status] ?? "·");
